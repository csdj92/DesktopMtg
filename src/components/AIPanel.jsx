...
const AIPanel = ({ deck, commander, onCardSuggestion, onClose, recommendations, deckArchetype, deckAnalysis, format }) => {
  ...
  const initializeLLM = async () => {
    if (globalLlmInitializing) {
      // Wait for the other instance to finish initializing
      const checkInitialized = setInterval(() => {
        if (globalLlmInitialized || !globalLlmInitializing) {
          setLlmInitialized(globalLlmInitialized);
    ...
    try {
      ...
      try {
        ...
        await window.electronAi.prompt('test', { timeout: 2000 });
        globalLlmInitialized = true;
        setLlmInitialized(true);
        console.log('✅ LLM already initialized');
        return;
      } catch (testError) {
        // Model not ready, need to create it
        console.log('🔄 Creating new LLM instance...');
      }

      // Create the model instance with proper error handling
      try {
        console.log('🔄 Attempting to create LLM model...');
        await Promise.race([
          window.electronAi.create({
            modelAlias: 'Meta-Llama-3-8B-Instruct.Q4_K_M.gguf',
            systemPrompt: 'You are a Magic: The Gathering deck analysis assistant. Provide helpful, accurate advice about deck building, card synergies, and strategy. Focus on practical recommendations and explain your reasoning.',
            temperature: 0.7,
            topK: 10
          }),
          new Promise((_, reject) => 
            setTimeout(() => reject(new Error('Model creation timed out after 60 seconds')), 60000)
          )
        ]);
        
        // Test the model works
        console.log('🔄 Testing model functionality...');
        await window.electronAi.prompt('Hello', { timeout: 5000 });
        
        globalLlmInitialized = true;
        setLlmInitialized(true);
        console.log('✅ LLM initialized successfully');
      } catch (createError) {
        console.error('❌ Failed to create LLM model:', createError);
        if (createError.message.includes('modelAlias')) {
          throw new Error('Model file not found. Please ensure the AI model is downloaded.');
        } else {
          throw new Error(`Failed to create LLM model: ${createError.message}`);
        }
      }
    } catch (error) {
